...
public class FoodGen : MonoBehaviour
{
    ...
    int indexSpawnFood = 10; // The index used in the pathtofollow to generate the food more than 4 blocks away from each other
        ...
        if (loop) 
        {
            while (produceFood)
            {
                ...
                for (int i = 0; i <= posn.Count; i++) // looping throught the path list created by the seaker 
                {
                    ...
                    indexSpawnFood += Random.Range(4, 10);// Adding to our index so that the food spawns a min of 4 blocks away up to 10 blocks away. 
